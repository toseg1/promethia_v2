...
class ProfileService {
  ...
  async updateProfile(userId: string, profileData: ProfileUpdateData): Promise<UserProfileBundle> {
    ...
    if (profileData.countryCode !== undefined) {
      // Convert country code to dial code - complete mapping from PhoneInput
      ...
      const dialCode = countryDialCodes[profileData.countryCode] || '+33';
      payload.countryNumber = dialCode;
      console.log('ProfileService: Converting country code', profileData.countryCode, 'to dial code', dialCode);
    }
